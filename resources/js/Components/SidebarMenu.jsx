        ...
        <Fragment>
            <li>
                <Menu>
                    {({open}) => (
                        <>
                            <Menu.Button className={`item-menu cursor-pointer focus:ring-2 focus:ring-blue-200 text-slate-700 transition-all group py-2 px-3 rounded-md  flex items-center gap-1 ${active ? 'text-slate-50 font-medium bg-blue-600 hover:bg-blue-600 border-b-2 backdrop-blur border-b-blue-700' : 'hover:bg-blue-50'} group w-full`}>
                                <div
                                ...
                                <IconCaretDownFilled size={16} className={`${open ? 'rotate-[270deg]' : ''} transition-all duration-500 text-slate-500 group-hover:text-blue-500`} stroke={1}/>
                            </Menu.Button>
                            {open && (
                                <Menu.Items static className={`${open ? 'rounded-md' : ''} ${isExpanded ? 'fixed bg-white border-slate-400 px-5 ps-0 py-4 pb-2 z-50 left-20 shadow-sm  rounded-sm' : 'py-2 space-y-2 '}  `}>
                                    {children}
                                </Menu.Items>
                            )}
                        </>
